(() => {
    // @ts-expect-error TODO: Figure out a better way
    ...
    async function orderPage() {
        ...
        const matches = document
            ...
            .match(dateRegex);
        if (!matches) {
            throw Error("failed to find a date from the 'order paid' stepper");
        }
        const year = Number.parseInt(matches[3]);
        const month = Number.parseInt(matches[2]);
        const day = Number.parseInt(matches[1]);
